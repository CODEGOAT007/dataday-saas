// Script to manually send consent requests to all pending emergency support team members
...
async function sendConsentRequests() {
  console.log('📧 Sending Consent Requests to Emergency Support Team')
  console.log('===================================================')
  
  try {
    // For now, let's manually send to Michael Loy since we know he needs consent
    const testMember = {
      id: 'manual-test-id',
      name: 'Michael Loy',
    ...
    const user = {
      full_name: 'Christopher Loy',
      email: 'christopher.j.loy@gmail.com'
    }
    
    console.log(`📧 Sending consent request to: ${testMember.name} (${testMember.email})`)
    
    const consentUrl = `${process.env.NEXT_PUBLIC_APP_URL}/consent/${testMember.id}`
    const subject = `${user.full_name} added you to their Emergency Support Team`
    
    const message = `Hi ${testMember.name}!
...
- The MyDataday Team`
    ...
    console.log(`📬 Check ${testMember.email} for the consent request`)
